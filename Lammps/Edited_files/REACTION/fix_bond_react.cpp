  ...
  if (ifit >= 0 && ifit < atom->nlocal) {
    ...
    for (int j = 0; j < twomol->natoms; j++) {
      ...
      if (!create_atoms[j][rxnID] && !delete_atoms[ipre][rxnID]) {
        ...
        else if (modify_create_nucrand[rxnID] > 2) { // Put new positions as two spherical asters separated by argument  -- Billie 04/01/24 // Edit such that nucleation is bounded by an ellipsoid 17/06/2024
          double s = 1;
          double u1 = random[rxnID]->uniform();
  ...
  if (overlapsq[rxnID] > 0) {
    ...
    for (int m = 0; m < twomol->natoms; m++) {
      ...
      if (create_atoms[m][rxnID] == 1) {
        ...
        if (((coords[m][0]*coords[m][0])/(100*100)+(coords[m][1]*coords[m][1])/(70*70)+(coords[m][2]*coords[m][2])/(70*70)) >0.95) { //if this is aster configuration, make the aster bounded by an ellipsoid Billie 17/06/2024 equation of ellipsoid (modify_create_nucrand[rxnID] > 2 -- but have to just do generally 
            abortflag = 1;
            break;
        }  
        for (int i = 0; i < nlocal; i++) {
