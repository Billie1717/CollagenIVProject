  ...
  broken(nullptr), copy(nullptr), random(nullptr)
{
  ...
  nevery = utils::inumeric(FLERR,arg[3],false,lmp); //Reading in the arguments (Nevery)
  ...
  btype = utils::inumeric(FLERR,arg[4],false,lmp); //Reading in the arguments (Bond type)
  cutoff = utils::numeric(FLERR,arg[5],false,lmp); //cutoff distance to break the bond
  ...
  cutsq = cutoff*cutoff;  //from now on using cutsq
  ...
  BondEChem = 0.0;
  EquibCounter = 0;
  fmt::print("before args {}", restlength);
  while (iarg < narg) {
    if (strcmp(arg[iarg],"prob") == 0) { 
      //if (iarg+3 > narg) error->all(FLERR,"Illegal fix bond/break command"); //CHANGED  - Billie 28/02/24
      fractionIn = utils::numeric(FLERR,arg[iarg+1],false,lmp); //Reading in the arguments (probability to break)
      ...
      iarg += 3;
    } else if (strcmp(arg[iarg],"equilib") == 0) { //-------------- Adding in option for equilibrium bond breaking ------------ Billie 28/02/2024//
        if (iarg+4 > narg) error->all(FLERR,"Illegal fix bond/break command");
        restlength = utils::numeric(FLERR,arg[iarg+1],false,lmp); 
        bondconstant = utils::numeric(FLERR,arg[iarg+2],false,lmp);
        BondEChem = utils::numeric(FLERR,arg[iarg+3],false,lmp);
        EquibCounter = 1;
        iarg += 4;
    }
    //else error->all(FLERR,"Illegal fix bond/break command");
  }
  if (EquibCounter == 0){fraction = fractionIn;}
  //fmt::print("{}", bondconstant);
  fmt::print("after args {}", restlength);
...
void FixBondBreak::post_integrate()
{
  ...
  tagint *tag = atom->tag;
  int *mask = atom->mask;
  int **bondlist = neighbor->bondlist; //What is the structure of bond list?
  int nbondlist = neighbor->nbondlist;

  for (n = 0; n < nbondlist; n++) {
    i1 = bondlist[n][0]; //first atom in nth bond
    i2 = bondlist[n][1]; //second atom in nth bond
    type = bondlist[n][2]; //bondtype
    ...
    dely = x[i1][1] - x[i2][1];
    delz = x[i1][2] - x[i2][2];
    rsq = delx*delx + dely*dely + delz*delz;  //bond length square! but this is re-calculated -- stored in distsq[i2],distsq[i1]?
    if (EquibCounter==0){
        if (rsq <= cutsq){
            continue;}}  //When no equilibrium values are specified, just ignore things that are below cutoff Billie 28/02/2024

    if (rsq > distsq[i1]) {  //this is calculating "winning" partner : longest bonds, st only 1 per atom is broken for any one instance
      partner[i1] = tag[i2];
      distsq[i1] = rsq;
    }
    if (rsq > distsq[i2]) {
  ...
  //if (fraction < 1.0) { Billie - 28/02/24 Removed condition (want the value to always be generated)
  for (i = 0; i < nlocal; i++)
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    //BondEChem = (restlength - restlength*2.5)*(restlength - restlength*2.5)*bondconstant;
    if (EquibCounter==0){fraction = fractionIn;}
    else{
        BondEnergy = (restlength-sqrt(distsq[i]))*(restlength-sqrt(distsq[i]))*bondconstant-BondEChem; //Billie = Are we sure we need distsq[i]? And not distsq[j]?
    //BondEcutoff = (restlength-restlength*3)*(restlength-restlength*3)*bondconstant;
    // apply probability constraint using RN for atom with smallest ID
        if ((BondEnergy) >=0){fraction = fractionIn;} //Break with probabilty 1 if Bond energy is larger than cutoff (length 1.5, unlikely!)
        else {fraction = fractionIn*exp(BondEnergy);}} //Choosing cutoff where it will definitely break as 1.5 (3xR0) as if I look at bond distributions, there are very few with a larger distance than this. Splitting this exp() dependency in half, half going to the probability to make a bond.
    //fraction = exp(-(distsq[i]); //we don't have the bond energy here or the bond rest length... Add as an argument?
