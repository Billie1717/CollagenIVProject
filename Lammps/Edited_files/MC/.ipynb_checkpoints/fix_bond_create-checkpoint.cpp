  ...
  probability(nullptr), created(nullptr), copy(nullptr), random(nullptr), list(nullptr)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"iparam") == 0) {
      ...
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix bond/create command");
      fractionIn = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"equilib") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix bond/create command");
  ...
  if (restlength==0){fraction = fractionIn;}
...
void FixBondCreate::post_integrate()
{
  ...
  for (i = 0; i < nlocal; i++) {
   ...
   // BondEChem = (restlength - restlength*2.5)*(restlength - restlength*2.5)*bondconstant;
    ...
    if (restlength!=0.0){
        if ((BondEnergy) >=0){fraction = fractionIn*exp(-BondEnergy);} //Break with probabilty 1 if Bond energy is negative, else create with prob e^-En)
        else {fraction = fractionIn;}}
    if (restlength==0.0){fraction = fractionIn;}
    if (fraction < 1.0) {
      if (tag[i] < tag[j]) {
...
void FixBondCreate::create_angles(int m)
{
  ...
  for (i = 0; i < n2; i++) {
    ...
    for (j = i+1; j < n2; j++) {
      i3 = s2list[j];
      billieflag = 0; //flag for if any of this i,j is NOT bond 1-1-2 Billie 20/12/2023
      // angle = i1-i2-i3
    ...
    for (int l = 0; l < atom->natoms; l++) {
            if (tag[l] == i1)
                index1 = l;
            else if (tag[l] == i3)
                index3 = l;
            //if (index1!=-1 && index3!=-1)
              //  break; Including this messes things up!
      }
     if (atom->type[index1]>=2 && atom->type[index3]>=2) billieflag = 1; 
    if (billieflag ==0){
